  ...
  try {
    ...
    for (const operation of deleteOperations) {
      try {
        ...
        // 忽略表不存在的错误
        logger.warn('清理表时出现错误（可能是表不存在）:', err);
      }
    }
  ...
  return await batchInsert('ArmorSet', armorSets, async (batch) => {
    await prisma.armorSet.createMany({
      data: batch.map((armorSet: any) => ({
        ...
        names: JSON.stringify(armorSet.names || {}),
        rarity: armorSet.rarity || 0,
        set_bonus: armorSet.set_bonus ? JSON.stringify(armorSet.set_bonus) : null,
        group_bonus: armorSet.group_bonus ? JSON.stringify(armorSet.group_bonus) : null,
        pieces: JSON.stringify(armorSet.pieces || []),
      })),
      skipDuplicates: true,
    });
  });
...
// 导入护石数据
async function importAmulets(): Promise<ImportStats> {
  const rawData = loadJsonFile(DATA_FILES.amulets);
  const amulets = validateAndCleanData(rawData, ['game_id'], 'Amulet');
  
  return await batchInsert('Amulet', amulets, async (batch) => {
    await prisma.amulet.createMany({
      data: batch.map((amulet: any) => ({
        game_id: BigInt(amulet.game_id),
        ranks: JSON.stringify(amulet.ranks || []),
      })),
      skipDuplicates: true,
    });
  });
